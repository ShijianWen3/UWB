...
int residual_func(const gsl_vector *x, void *data, gsl_vector *f) {
    ...
    position_t *anchors = user_data->anchors;
    int *distances = user_data->distances;
...
void callback(const size_t iter, void *params, const gsl_multifit_nlinear_workspace *w)
{
    ...
    qDebug() << "iter" << iter << ": X =" << gsl_vector_get(x, 0) << ", Y =" << gsl_vector_get(x, 1) << ", cond(J) =" << 1.0 / rcond << ", |f(x)| =" << gsl_blas_dnrm2(f);
...
//        size_t n = sizeof(anchors) / sizeof(anchors[0]);
        printf("基站数量%d\n",n);
//        weights = {1.0, 1.0, 1.0, 1.0}; // 示例权重，可以根据需要调整
        ...
        best_solution->x = gsl_vector_get(x_opt, 0)/100.0f;
        best_solution->y = gsl_vector_get(x_opt, 1)/100.0f;
        best_solution->z = gsl_vector_get(x_opt, 2)/100.0f;
...
int GetLocation(vec3d *best_solution, vec3d* anchorArray, int *distanceArray, int dimensional)
{
    ...
    else if(dimensional == 1)
    {
        ...
        else //valid_anc_count >= 2
        {
            ...
            //findPointA(best_solution,1.7,0.0,0.0,0.0,1.0,2.7);
...
int GetLocation(vec3d *best_solution, vec3d* anchorArray, int *distanceArray)
{
    ...
    if (result >= 0)
    {
        ...
        qDebug() << "O1"<<"x="<<o1.x << "y="<< o1.y<< "z=" <<o1.z;
        qDebug() << "O2"<<"x="<<o2.x << "y="<< o2.y<< "z=" <<o2.z;

        qDebug() << "solution"<<"x="<<best_solution->x << "y="<< best_solution->y<< "z=" <<best_solution->z;
