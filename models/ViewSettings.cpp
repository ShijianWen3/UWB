...
double ViewSettings::tagSize() const
{
    return m_tagSize;
}

void ViewSettings::setTagSize(double newTagSize)
{
    if (qFuzzyCompare(m_tagSize, newTagSize))
        return;
    m_tagSize = newTagSize;
    emit tagSizeChanged();
}

int ViewSettings::canvesFontSize() const
{
    return m_canvesFontSize;
}

void ViewSettings::setCanvesFontSize(int newCanvesFontSize)
{
    if (m_canvesFontSize == newCanvesFontSize)
        return;
    m_canvesFontSize = newCanvesFontSize;
    emit canvesFontSizeChanged();
    emit fontSizeChangedeSignal(m_canvesFontSize);
}
