    ...
    _floorplanOpen(false)
{
   ...
   // QObject::connect(ui->showGeoFencingMode, SIGNAL(clicked()), this, SLOT(showGeoFencingModeClicked()));
    ...
    //add by ec
    QObject::connect(ui->checktagwarn, SIGNAL(stateChanged(int)), this, SLOT(checkTagWarnClicked(int)));
...
//add by ec
void ViewSettingsWidget::checkTagWarnClicked(int state)
{
    if(state == Qt::Checked)  //选中
    {
        qDebug() << "CheckBox is clicked true ";
        //ui->tagWarnSize->setEnabled(true);
        qDebug() << "tagWarnSize is: " << ui->tagWarnSize->text().toInt();
    }
    else
    {
        qDebug() << "CheckBox is clicked false ";
        //ui->tagWarnSize->setDisabled(true);

    }
    RTLSDisplayApplication::graphicsWidget()->checktagwarn(state, ui->tagWarnSize->text().toInt());
}
...
bool ViewSettingsWidget::checkAnchorStatus(int r){
    QTableWidgetItem *xItem = myGraphicsWidget->ui->anchorTable->item(r, 3);
    QTableWidgetItem *yItem = myGraphicsWidget->ui->anchorTable->item(r, 5);
    QTableWidgetItem *zItem = myGraphicsWidget->ui->anchorTable->item(r, 4);
...
void ViewSettingsWidget::updateAnchorList3D(int ridx, bool status,bool update,double x, double y,double z){
}
void ViewSettingsWidget::updateAncAndLabel(int r, bool update=false){



    double x = myGraphicsWidget->ui->anchorTable->item(r, 3)->text().toDouble();
    double y = myGraphicsWidget->ui->anchorTable->item(r, 5)->text().toDouble();
    double z = myGraphicsWidget->ui->anchorTable->item(r, 4)->text().toDouble();
    int ancid = myGraphicsWidget->ui->anchorTable->item(r, 1)->text().toInt();
    int groupid = myGraphicsWidget->ui->anchorTable->item(r, 0)->text().toInt();

    QVector3D position(x,y,z);
    if(position == series->dataProxy()->itemAt(r)->position())
        return;
    if(scatter->customItems().contains(m_labelAnc[r])){
        scatter->removeCustomItem(m_labelAnc[r]);
    }
    m_labelAnc[r] = new QCustom3DLabel();
    if (r<series->dataProxy()->itemCount())
    ...
    QString labelText = QString("ANC %1:%2 (%3, %4, %5)")
            .arg(groupid).arg(ancid).arg(QString::number(x, 'f', 2))
...
//    ancDataArray = new QScatterDataArray;
//    ancDataArray->resize(10);
//    series->dataProxy()->resetArray(ancDataArray);
    series->setMesh(QAbstract3DSeries::MeshPyramid);
    series->setItemSize(0.3f*ratio);

    ancDataArray = new QScatterDataArray;
    ancDataArray->resize(0);
//    QScatterDataItem  *ptrToAncDataArray = &ancDataArray->first();
    int index = 0;
    for (BaseStation &baseStation : baseStationList)
    {
        if (checkAnchorStatus(index))
            updateAncAndLabel(index);
        index ++;
    }
    for(int i=0 ; i< MAX_NUM_TAGS; i++ ){
        seriesPoint[i]->setMesh(QAbstract3DSeries::MeshCube);
        seriesPoint[i]->setMeshSmooth(false);
        seriesPoint[i]->setItemSize(0.1f*ratio);
    }
...
void ViewSettingsWidget::updateAxisRange(bool tag=false)
{
    ...
    {    // 重置最大最小值
        ...
        minX = 0;
        minY = 0;
        minZ = 0;//std::numeric_limits<double>::max();
    }

    for (int i = 0; i < myGraphicsWidget->ui->anchorTable->rowCount(); ++i) {
        //        QVector3D itemPos = series->dataProxy()->itemAt(i)->position();
        if (checkAnchorStatus(i))
        {
            double x = myGraphicsWidget->ui->anchorTable->item(i, 3)->text().toDouble();
            double y = myGraphicsWidget->ui->anchorTable->item(i, 5)->text().toDouble();
            double z = myGraphicsWidget->ui->anchorTable->item(i, 4)->text().toDouble();
    ...
    if (updateRange)
    {
        scatter->setHorizontalAspectRatio(maxX/maxZ);//水平纵横比是x轴与z轴之间的图形比例，默认0.0，表示根据轴范围自动缩放
        scatter->setAspectRatio((maxX>maxZ?maxX:maxZ)/maxY);//设置水平面最长轴与y轴之间的图形比例，默认为2
