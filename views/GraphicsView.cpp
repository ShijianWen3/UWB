...
static qreal lastAngle = 0.0;
static qreal mouseAngle = 0.0;
...
void GraphicsView::scaleView(qreal sx, qreal sy, QPointF center)
{
    ...
    visibleRect.setWidth(qBound(1., visibleRect.width() * sx, 1500.));
    visibleRect.setHeight(qBound(1., visibleRect.height() * sy, 1500.));
    visibleRect.translate(-diff.x() * visibleRect.width() / this->_visibleRect.width(),
                          -diff.y() * visibleRect.height() / this->_visibleRect.height());

    qreal scale = qBound(1., visibleRect.height() * sy, 1500.);
...
void GraphicsView::centerRect(const QRectF &visibleRect)
{
    ...
    //容错，当rect为一条直线时，center会出现nan，导致矩形设置错误。手动添加个宽度
    QRectF rec = visibleRect;
    if(rec.width() == 0)
    {
        rec.setWidth(1);
    }
...
void GraphicsView::mousePressEvent(QMouseEvent *event)
{
    ...
    g_button = event->button();
    centerPoint = mapToParent(this->viewport()->rect().center());
    startPoint = mapToParent(event->pos());
...
void GraphicsView::mouseMoveEvent(QMouseEvent *event)
{
    ...
    case DefaultMouseContext:
        ...
        {
            currentPoint = mapToParent(event->pos());
            QLineF lineBegin(centerPoint, startPoint);
            QLineF lineEnd(centerPoint, currentPoint);
            mouseAngle = lineBegin.angleTo(lineEnd);
            qreal angel = mouseAngle-lastAngle;
            QString angelStr = QString::number(angel, 'f', 2);
            if(mouseAngle > lastAngle && abs(angel) < 350 && firstMove == false)
            {
                rotateAngle+=angel;
                this->rotate(angelStr.toFloat());
            }
            else if(mouseAngle < lastAngle && abs(angel) < 350 && firstMove == false)
            {
                rotateAngle+=angel;
                this->rotate(angelStr.toFloat());
            }
            ...
            this->update();
            emit rotateChanged(rotateAngle);
...
void GraphicsView::drawFloorplan(QPainter *painter, const QRectF &rect)
{
    ...
    {
        painter->save();
        QTransform transform = RTLSDisplayApplication::viewSettings()->floorplanTransform();
